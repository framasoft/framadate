...
class PollService {
    ...
    function allSlotsByPoll(Poll $poll) {
        $slots = $this->slotRepository->listByPollId($poll->getId());
        if ($poll->getFormat() === 'D') {
     ...
     * @return array
     */
    public function createPoll(Poll $form) {
        // Generate poll IDs, loop while poll ID already exists

        if (empty($form->getId())) { // User want us to generate an id for him
            do {
    ...
    private function checkMaxVotes($user_choice, Poll $poll, $poll_id) {
        ...
        foreach ($best_choices['y'] as $i => $nb_choice) {
            ...
            if ($nb_choice >= $poll->getValueMax() && $user_choice[$i] === "2") {
