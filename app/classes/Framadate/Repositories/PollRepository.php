...
class PollRepository extends AbstractRepository {
    ...
    function update($poll) {
        $prepared = $this->prepare('UPDATE `' . Utils::table('poll') . '` SET title=?, admin_name=?, admin_mail=?, description=?, end_date=?, active=?, editable=?, hidden=?, password_hash=?, results_publicly_visible=?, closed=?, admin_choice=?, admin_choice_exp=? WHERE id = ?');

        return $prepared->execute([$poll->title, $poll->admin_name, $poll->admin_mail, $poll->description, $poll->end_date, $poll->active, ($poll->editable>=0 && $poll->editable<=2) ? $poll->editable  : 0, $poll->hidden ? 1 : 0, $poll->password_hash, $poll->results_publicly_visible ? 1 : 0, $poll->closed ? 1 : 0, $poll->admin_choice ? $poll->admin_choice : "", $poll->admin_choice_exp ? $poll->admin_choice_exp : "", $poll->id]);
    }
