    ...
    {if $comments|count > 0}
        ...
        {foreach $comments as $comment}
            <form action="{if $admin}{poll_url id=$admin_poll_id admin=true}{else}{poll_url id=$poll_id}{/if}" method="POST">
                <div class="comment" data-comment-id="{$comment->id|html}">
                    {if $admin && !$expired}
                    ...
                    {if $admin && !$expired}
                        <div class="hidden js-comment">
                            ...
                            <div class="input-group">
                                <textarea class="form-control" id="editComment" name="editComment" rows="2" cols="40" required>{$comment->comment|html}</textarea>
                                <input type="hidden" id="edit_id" name="edit_id" value="{$comment->id|html}" />

                                <span class="input-group-btn">
                                <button type="submit" class="btn btn-success" name="edit_comment" value="comment"  title="{__('PollInfo', 'Save the new com')}">
                                    <span class="glyphicon glyphicon-ok"></span><span class="sr-only">{__('Generic', 'Save')}</span>
                                </button>
                                <button class="btn btn-link btn-cancel" title="{__('PollInfo', 'Cancel the name edit')}">
                                    <span class="glyphicon glyphicon-remove"></span><span class="sr-only">{__('Generic', 'Cancel')}</span>
                                </button>
                            </span>
                            </div>
                        </div>
